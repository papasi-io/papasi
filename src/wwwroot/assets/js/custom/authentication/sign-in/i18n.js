...
var SwapAuthI18nDemo = function() {
        ...
        for (var label in translationStrings) {
			if (translationStrings.hasOwnProperty(label)) {
				if (translationStrings[label][lang]) {
					let labelElement = document.querySelector('[data-swap-translate=' + label + ']');
	...
	var setLanguage = function(lang) {
		const selectedLang = menu.querySelector('[data-swap-lang="' + lang + '"]');

		if (selectedLang !== null) {
			const currentLangName = document.querySelector('[data-swap-element="current-lang-name"]'); 
			const currentLangFlag = document.querySelector('[data-swap-element="current-lang-flag"]'); 

			const selectedLangName = selectedLang.querySelector('[data-swap-element="lang-name"]');
			const selectedLangFlag = selectedLang.querySelector('[data-swap-element="lang-flag"]');

			currentLangName.innerText = selectedLangName.innerText;
			currentLangFlag.setAttribute("src", selectedLangFlag.getAttribute("src"));

			localStorage.setItem("swap_auth_lang", lang);
		}
	}

	var init = function() {
		if ( localStorage.getItem("swap_auth_lang") !== null ) {
			let lang = localStorage.getItem("swap_auth_lang");
		...
		menuObj.on("kt.menu.link.click", function(element) {
			let lang = element.getAttribute("data-swap-lang");
    ...
    return {
        // Initialization
        init: function() {
            menu = document.querySelector('#swap_auth_lang_menu');

			if (menu === null) {
				return;
			} 

			menuObj = SwapMenu.getInstance(menu);
