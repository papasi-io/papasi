...
// Class definition
var SwapAppInboxReply = function () {

    // Private functions
    const handlePreviewText = () => {
        // Get all messages
        const accordions = document.querySelectorAll('[data-swap-inbox-message="message_wrapper"]');
        accordions.forEach(accordion => {
            // Set variables
            const header = accordion.querySelector('[data-swap-inbox-message="header"]');
            const previewText = accordion.querySelector('[data-swap-inbox-message="preview"]');
            const details = accordion.querySelector('[data-swap-inbox-message="details"]');
            const message = accordion.querySelector('[data-swap-inbox-message="message"]');
            ...
            header.addEventListener('click', e => {
                // Return if SwapMenu or buttons are clicked
                if (e.target.closest('[data-swap-menu-trigger="click"]') || e.target.closest('.btn')) {
                    return;
                } else {
    ...
    const initForm = () => {
        // Set variables
        const form = document.querySelector('#swap_inbox_reply_form');
        const allTagify = form.querySelectorAll('[data-swap-inbox-form="tagify"]');
    ...
    const handleCCandBCC = (el) => {
        // Get elements
        const ccElement = el.querySelector('[data-swap-inbox-form="cc"]');
        const ccButton = el.querySelector('[data-swap-inbox-form="cc_button"]');
        const ccClose = el.querySelector('[data-swap-inbox-form="cc_close"]');
        const bccElement = el.querySelector('[data-swap-inbox-form="bcc"]');
        const bccButton = el.querySelector('[data-swap-inbox-form="bcc_button"]');
        const bccClose = el.querySelector('[data-swap-inbox-form="bcc_close"]');
    ...
    const handleSubmit = (el) => {
        const submitButton = el.querySelector('[data-swap-inbox-form="send"]');
    ...
    const initTagify = (el) => {
        ...
        const usersList = [
            { value: 1, name: 'Emma Smith', avatar: 'avatars/300-1.jpg', email: 'e.smith@kpmg.com.au' },
    ...
    // Init quill editor 
    const initQuill = (el) => {
        var quill = new Quill('#swap_inbox_form_editor', {
            modules: {
                toolbar: [
                    [{
                        header: [1, 2, false]
    ...
    const initDropzone = (el) => {
        ...
        const id = '[data-swap-inbox-form="dropzone"]';
        const dropzone = el.querySelector(id);
        const uploadButton = el.querySelector('[data-swap-inbox-form="dropzone_upload"]');
        ...
        var myDropzone = new Dropzone(id, { // Make the whole body a dropzone
            url: "https://preview.papasi.io/api/dropzone/void.php", // Set the url for your upload script location
            parallelUploads: 20,
