...
// Class definition
var SwapModalTwoFactorAuthentication = function () {
    // Private variables
    var modal;
    var modalObject;
			...
			if (smsValidator) {
				smsValidator.validate().then(function (status) {
					...
					if (status == 'Valid') {
						...
						smsSubmitButton.setAttribute('data-swap-indicator', 'on');
						...
						setTimeout(function() {
							...
							smsSubmitButton.removeAttribute('data-swap-indicator');
        ...
        appsSubmitButton.addEventListener('click', function (e) {
			...
			if (appsValidator) {
				appsValidator.validate().then(function (status) {
					...
					if (status == 'Valid') {
						appsSubmitButton.setAttribute('data-swap-indicator', 'on');

						// Disable button to avoid multiple click 
						appsSubmitButton.disabled = true;

						setTimeout(function() {
							appsSubmitButton.removeAttribute('data-swap-indicator');
    ...
    return {
        init: function () {
            // Elements
            modal = document.querySelector('#swap_modal_two_factor_authentication');
            ...
            optionsWrapper = modal.querySelector('[data-swap-element="options"]');
            optionsSelectButton = modal.querySelector('[data-swap-element="options-select"]');

            smsWrapper = modal.querySelector('[data-swap-element="sms"]');
            smsForm = modal.querySelector('[data-swap-element="sms-form"]');
            smsSubmitButton = modal.querySelector('[data-swap-element="sms-submit"]');
            smsCancelButton = modal.querySelector('[data-swap-element="sms-cancel"]');

            appsWrapper = modal.querySelector('[data-swap-element="apps"]');
            appsForm = modal.querySelector('[data-swap-element="apps-form"]');
            appsSubmitButton = modal.querySelector('[data-swap-element="apps-submit"]');
            appsCancelButton = modal.querySelector('[data-swap-element="apps-cancel"]');
